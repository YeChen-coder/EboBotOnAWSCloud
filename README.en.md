...
The target is one Linux x86_64 Fargate task in Canada Central (`ca-central-1`) with 1 vCPU and 2 GiB of memory. It contains `ebo-engine` and `realtime-assistant`; Home Assistant stays on the local Windows machine. See the full architecture study in [English](EBO_Fargate_Architecture_and_Local_Diagnostic_Agent_Report.en.md) or [Chinese](EBO_Fargate架构与诊断Agent调研报告.md).

## Current status

The memory reduction completed on 2026-09-12. Roughly 65 hours of historical observations showed a task memory peak near 320 MiB, so the authorized configuration change reduced task memory from 4 GiB to 2 GiB. Task definition `ebo-cloud-lab:6` passed its final check at 20:31 UTC: CloudFormation was `UPDATE_COMPLETE`, the ECS deployment was `COMPLETED`, both containers were `HEALTHY`, and audio, video, and Realtime remained normal after the startup grace period. The highest sample during the first three minutes was 277 MiB. Estimated Fargate compute is about USD 39.63 per 730-hour month, saving about USD 7.10. See the [memory sizing assessment](Fargate内存降配评估_2026-09-12.md). Deployment-specific task identifiers are redacted from this repository.

The conversation logging enhancement completed on 2026-09-11. The Assistant now emits final user transcripts, final model output, and key diagnostic events as structured CloudWatch logs. Fifty-five Assistant tests, eight cloud tests, and an offline image-path check passed. The deployed service produced the new `realtime.connected` event, and a short-lived synthetic task verified that full Chinese transcript and response text reached CloudWatch. The production service had only one empty transcript observation at that time, so a real spoken exchange still requires human validation. See the [CloudWatch conversation log guide](cloud/CloudWatch对话日志查看指南.md).

The CPU reduction completed on 2026-09-10. CloudFormation deployed 1 vCPU and 4 GiB as task definition `ebo-cloud-lab:3`. The first replacement task encountered an Enabot login connection timeout; an identical replacement recovered. Both containers then became healthy, the ECS deployment completed, and audio input and Realtime connectivity were normal after the grace period. See the [AWS cost estimate](AWS成本估算_2026-09-10.md).

The initial cloud cutover completed on the evening of 2026-09-09 in Toronto time. CloudFormation reached `UPDATE_COMPLETE`; the ECS service held one task; both business containers were healthy; and real audio, video, and Realtime connectivity worked. The old local Engine and Assistant were stopped, while local Home Assistant stayed running. The user could not perform the final listening and speaking test at that time. See the [deployment acceptance record](cloud/部署验收记录.md) and [permission record](cloud/权限与部署确认.md).
...
- Each business container has its own EFS access point. The initial import preserves microphone and UI choices but does not copy historical family conversations, recordings, or images.
...
6. For the first cutover, optionally run `cloud/preflight.py launch`, then confirm with `cloud/preflight.py status` that both containers exit with code 0 after reading secrets and EFS. Stop the local Assistant and Engine and confirm they exited before running `cloud/deploy.py start`. Home Assistant remains running.
